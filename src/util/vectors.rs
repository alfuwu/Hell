...
impl DivAssign<Vector3f> for Vector3f {
    fn div_assign(&mut self, rhs: Self) { self.x /= rhs.x; self.y /= rhs.y; self.z /= rhs.z; }
}
impl<T> Add<T> for Vector3f where T: Into<f32> {
    type Output = Self;
    fn add(self, rhs: T) -> Self::Output { let rhs = rhs.into(); Self { x: self.x + rhs, y: self.y + rhs, z: self.z + rhs } }
}
impl<T> Sub<T> for Vector3f where T: Into<f32> {
    type Output = Self;
    fn sub(self, rhs: T) -> Self::Output { let rhs = rhs.into(); Self { x: self.x - rhs, y: self.y - rhs, z: self.z - rhs } }
}
impl<T> Mul<T> for Vector3f where T: Into<f32> {
    type Output = Self;
    fn mul(self, rhs: T) -> Self::Output { let rhs = rhs.into(); Self { x: self.x * rhs, y: self.y * rhs, z: self.z * rhs } }
}
...
impl<T> SubAssign<T> for Vector3f where T: Into<f32> {
    fn sub_assign(&mut self, rhs: T) { let rhs = rhs.into(); self.x -= rhs; self.y -= rhs; self.z -= rhs }
}
impl<T> MulAssign<T> for Vector3f where T: Into<f32> {
    fn mul_assign(&mut self, rhs: T) { let rhs = rhs.into(); self.x *= rhs; self.y *= rhs; self.z *= rhs; }
}
impl<T> DivAssign<T> for Vector3f where T: Into<f32> {
    fn div_assign(&mut self, rhs: T) { let rhs = rhs.into(); self.x /= rhs; self.y /= rhs; self.z /= rhs; }
}
impl PartialOrd for Vector3f {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> { self.length_squared().partial_cmp(&other.length_squared()) }
}
impl Index<usize> for Vector3f {
    type Output = f32;
    fn index(&self, i: usize) -> &Self::Output { match i { 0 => &self.x, 1 => &self.y, 2 => &self.z, _ => panic!("index out of bounds") } }
}
impl Display for Vector3f {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "({}, {}, {})", self.x, self.y, self.z) }
}
impl From<[f32; 3]> for Vector3f {
    fn from(v: [f32; 3]) -> Self { Self::new(v[0], v[1], v[2]) }
}
impl From<Vector3f> for [f32; 3] {
    fn from(v: Vector3f) -> Self { [v.x, v.y, v.z] }
}
